...
var startPlayer = {
    ...
    */
    buildings: [
        new Building(1.1, 0.1, 0), new Building(1.1, 25000, 0), new Building(1.3, 0, 3), new Building(1.1, 5, 0), new Building(1.05, 1000, 0),
    ...
    */
    currBuyables: [
        new CurrBuyable([10, 0, 0, 0, 0, 0]), new CurrBuyable([0, 50, 0, 0, 0, 0]), new CurrBuyable([0, 0, 100, 0, 0, 0]), new CurrBuyable([0, 0, 0, 100, 0, 0]), new CurrBuyable([0, 0, 0, 0, 100, 0]), new CurrBuyable([0, 0, 0, 0, 0, 100]),
...
function displayNum(num, ifMoney) {
    if (player.sciNotation)
        return Math.abs(num) < 100000 ? (ifMoney ? parseFloat(num.toString()).toFixed(2) : num.toString()) : parseFloat(num.toString()).toPrecision(5);

    for (var i = displayNumsuffixes.length - 1; i >= 0; i--) {
        if (Math.abs(num) >= Math.pow(10, 3 * i + 3) * 0.99999) {
            return (i < 4) ? parseFloat((num / Math.pow(10, 3 * i + 3)).toString()).toFixed(2) + displayNumsuffixes[i] : parseFloat((num / Math.pow(10, 3 * i + 3)).toString()).toFixed(2) + " " + displayNumsuffixes[i];
        }
    }
...
var calcMultfactors = [0.0005, 0.002, 0.005, 0.01, 0.02, 0.04, 0.06];
...
function importSave() {
    ...
    if (importText) {
        ...
        versionControl(true);
        if (player.upgrades[1] == 24)
            player.upgradeCosts[1] = Infinity;
        save();
        calcGlobalMult();
        if (player.numToBuy == -1)
            $("#currentNumToBuy").html("Max");
...
function ifMoreDerivs(tier) {
    if (player.numResets[tier - 1] == 0 && tier <= 3)
        return "Access to another derivative.\n";
    return "";
}
    ...
    if (player.buildings[4 + 5 * index].owned < 7000000000) {
        ...
        if (confirmation === true) {
            ...
            $.extend(true, player, {
                ...
                deriv1Money: 0.05,
                buildings: [
                    new Building(1.1, 0.1, 0), new Building(1.1, 25000, 0), new Building(1.3, 0, 2), new Building(1.1, 5, 0), new Building(1.05, 1000, 0),
                    new Building(1.2, 500, 0), new Building(1.3, 20000000, 0), new Building(1.8, 0, 20000), new Building(1.2, 1000, 0), new Building(1.2, 100000000, 0),
                    new Building(1.3, 20000, 0), new Building(1.8, 1000000000, 0), new Building(2.5, 0, 1000000), new Building(1.4, 100000, 0), new Building(1.5, 10000000000000, 0),
                    new Building(1.5, 1000000, 1000), new Building(2.5, 500000000000, 0), new Building(4, 0, 30000000), new Building(2, 10000000, 0), new Building(2, 1000000000000000000, 0),
                    new Building(2, 200000000, 500000), new Building(3, 20000000000000, 0), new Building(5, 0, 1000000000), new Building(2.7, 10000000000, 0), new Building(3, 100000000000000000000000, 0),
...
function updateInventory() {
    var firstRows = inventoryTemplate({
        deriv1Owned: displayNum(player.buildings[0].owned, false), deriv1Cost: displayNum(player.buildings[0].moneyCost, true), deriv1Manual: displayNum(player.buildings[0].manual, false), deriv1Power: displayNum(0.05 * player.mult[0] * globalMult[0], true),
    ...
    var row5 = row5Template({
        deriv5Owned: displayNum(player.buildings[20].owned, false), deriv5MoneyCost: displayNum(player.buildings[20].moneyCost, true), deriv5ProofCost: displayNum(player.buildings[20].proofCost, true), deriv5Manual: displayNum(player.buildings[20].manual, false), deriv5Power: displayNum(Math.round(player.mult[4] * globalMult[4]), false),
    ...
    var row6 = row6Template({
        deriv6Owned: displayNum(player.buildings[25].owned, false), deriv6MoneyCost: displayNum(player.buildings[25].moneyCost, true), deriv6ProofCost: displayNum(player.buildings[25].proofCost, true), deriv6Manual: displayNum(player.buildings[25].manual, false), deriv6Power: displayNum(Math.round(player.mult[5] * globalMult[5]), false),
    ...
    var row7 = row7Template({
        deriv7Owned: displayNum(player.buildings[30].owned, false), deriv7MoneyCost: displayNum(player.buildings[30].moneyCost, true), deriv7ProofCost: displayNum(player.buildings[30].proofCost, true), deriv7Manual: displayNum(player.buildings[30].manual, false), deriv7Power: displayNum(Math.round(player.mult[6] * globalMult[6]), false),
...
function updatePrestige() {
    var newPrestige = prestigeTemplate({
        tier1Resets: player.numResets[0], tier1ResetCurr: displayNum(player.resetCurr[0], false), tier1GlobalMult: displayNum(globalMult[0], false), tier1CurrTracker: displayNum(player.resetCurrTracker, false),
...
function updateResetCurrBuyables() {
    ...
    for (i = 0; i < buttonListProto.length; i++) {
        buttonList[(i % 4) * 6 + Math.floor(i / 4)] = buttonListProto[i];
    }

    if (typeof this.originalTitles == "undefined") {
...
function updateUpgrades() {
    var newUpgrades = upgradesTemplate({
        tier1UpgradeCost: displayNum(player.tierUpgradeCosts[0], true), tier1UpgradeOwned: displayNum(player.tierUpgrades[0], false), tier1Mult: displayNum(player.mult[0], false),
    ...
    if (!ifUnlockedTier(5))
        $("#tier5UpgradeRow").css('display', 'none');
    else
        $("#tier5UpgradeRow").css('display', 'table-row');
    if (!ifUnlockedTier(6))
        $("#tier6UpgradeRow").css('display', 'none');
    else
        $("#tier6UpgradeRow").css('display', 'table-row');
    if (!ifUnlockedTier(7))
        $("#tier7UpgradeRow").css('display', 'none');
    else
        $("#tier7UpgradeRow").css('display', 'table-row');
...
$(document).ready(function () {
    ...
    if (player.upgrades[1] == 24)
        player.upgradeCosts[1] = Infinity; //deals with JSON's incompatibility with Infinity
...
function buyCurrBuyable(index) {
    ...
    alert("click!");
    if (player.currBuyables[index].owned)
        return;
    if (index % 6 != 0) {
        if (!player.currBuyables[index - 1].owned)
            return;
    }
    for (var i = 0; i < numTiers - 1; i++)
        if (player.resetCurr[i] < player.currBuyables[index].cost[i])
            return;

    player.currBuyables[index].owned = true;
    for (var i = 0; i < numTiers - 1; i++)
        player.resetCurr[i] -= player.currBuyables[index].cost[i];
    calcGlobalMult();
...
var update = function () {
    ...
    do {
        ...
        if (player.proofsToNextCurr > -10000 * Math.pow(10 * (10 + player.proofsToCurrTracker), 6)) {
            ...
            var currToGain = Math.ceil(Math.pow(-player.proofsToNextCurr / (10000000 / 7) + Math.pow(10 + player.proofsToCurrTracker, 7), 1 / 7)) - 10 - player.proofsToCurrTracker;
            if (-player.proofsToNextCurr > 1e106)
                currToGain += currToGain / 1e6; //deals with inaccuracy in numbering
        ...
        if (player.mathematiciansToNextCurr > -7000000 * Math.pow(10 + player.mathematiciansToNextCurrTracker, 6)) {
            ...
            var currToGain = Math.ceil(Math.pow(-player.mathematiciansToNextCurr / 1000 + Math.pow(10 + player.mathematiciansToNextCurrTracker, 7), 1 / 7)) - 10 - player.mathematiciansToNextCurrTracker;
